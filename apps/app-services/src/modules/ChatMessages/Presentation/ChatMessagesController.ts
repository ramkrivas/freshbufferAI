...
import { ChatMessageRatingType, ChatType, IReactFlowObject } from '@app-services/core/Interfaces'
...
import { getChatflowById } from '../../ChatFlows/Application/UseCases/GetAllChatflows'
import {
    abortChatMessage,
    createChatMessage,
    getAllChatMessages,
...
} from '../Application/ChatMessagesUseCases'
...
const createChatMessageService = async (req: Request, res: Response, next: NextFunction) => {
    try {
        if (!req.body) {
        ...
        const apiResponse = await createChatMessage(req.body)
...
const getAllChatMessagesService = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const apiResponse = await getAllChatMessages(
            req.params.id,
            chatTypes,
            sortOrder,
            chatId,
...
const getAllInternalChatMessagesService = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const sortOrder = req.query?.order as string | undefined
        const chatId = req.query?.chatId as string | undefined
        const memoryType = req.query?.memoryType as string | undefined
        ...
        const apiResponse = await getAllInternalChatMessages(
            req.params.id,
            [ChatType.INTERNAL],
            sortOrder,
            chatId,
...
const removeAllChatMessagesService = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const chatflow = await getChatflowById(req.params.id)
        if (!chatflow) {
        ...
        if (!chatId) {
            ...
            const apiResponse = await removeChatMessagesByMessageIds(chatflowid, chatIdMap, messageIds)
            ...
            const apiResponse = await removeAllChatMessages(chatId, chatflowid, deleteOptions)
            return res.json(apiResponse)
        }
    } catch (error) {
        next(error)
...
const abortChatMessageService = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        await abortChatMessage(req.params.chatid, req.params.chatflowid)
...
export default {
    ...
    getAllChatMessagesService,
    getAllInternalChatMessagesService,
    removeAllChatMessagesService,
    abortChatMessageService
}
