...
import { FRESHBUFFERAI_CHATID, getBaseClasses, getCredentialData, getCredentialParam } from '../../../src/utils'
            ...
            for (let i = 0; i < flattenDocs.length; i += 1) {
                if (flattenDocs[i] && flattenDocs[i].pageContent) {
                    if (isFileUploadEnabled && options.chatId) {
                        flattenDocs[i].metadata = { ...flattenDocs[i].metadata, [FRESHBUFFERAI_CHATID]: options.chatId }
                    }
    ...
    async init(nodeData: INodeData, _: string, options: ICommonObject): Promise<any> {
        ...
        if (isFileUploadEnabled && options.chatId) {
            ...
            retrieverConfig.filter.should.push(
                {
                    key: `metadata.${FRESHBUFFERAI_CHATID}`,
                    match: {
